...
function buildSystemPrompt(ownerName: string, valueProp: string, today: string): string {
  return [
    ...
    `   - **Læses som om ${ownerName} har tastet det selv:** INGEN em dashes (—) eller en dashes (–), brug komma/punktum/kolon. INGEN ™/®. INGEN ALL-CAPS brand-staving. INGEN accenttegn — skriv "der"/"en"/"et", aldrig "dér"/"én"/"ét". Kort, casual dansk. Start med "Hej {fornavn}". Intet "/${ownerName}"-sign-off.`,
