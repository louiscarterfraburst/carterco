...
-- 3. Instant trigger: fire worker on cta_clicked / render_failed / played --
...
begin
    ...
    played_changed := new.played_at is not null
        and (tg_op = 'INSERT' or old.played_at is null);
    if not cta_changed and not fail_changed and not played_changed then
        return new;
    end if;
